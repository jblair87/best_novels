...
doc.search(".content__article-body").each do |new_novel|
  novel = BestNovels::Novel.new
  novel.title = new_novel.search("p:nth-child(1) strong a.u-underline").text.strip
  novel.novel_url = new_novel.search("a.u-underline").attr("href").value
  novel.summary = new_novel.search("p:nth-child(2)").text.gsub("/t"," ").gsub("/n"," ")
  #novels << new_novel
  novel.save
